/*
  # Authentication and Chat System Tables
...
CREATE POLICY "Users can see reactions in their rooms"
  ...
  USING (
    EXISTS (
      ...
      WHERE chat_messages.id = message_id AND chat_participants.user_id = auth.uid()
    )
  );
...
-- Create trigger to update updated_at on chat messages
